...
#if defined(__has_builtin) || (defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 3)))
...
static inline uint32_t crc32c_sw_big (uint32_t crc, void const *buf, size_t len)
{
	...
	if (len >= 8)
	{
		...
		do
		{
			...
			crcw = crc32c_table_big[0][crcw & 0xff] ^ crc32c_table_big[1][(crcw >> 8) & 0xff] ^ crc32c_table_big[2][(crcw >> 16) & 0xff] ^ crc32c_table_big[3][(crcw >> 24) & 0xff] ^ crc32c_table_big[4][(crcw >> 32) & 0xff] ^ crc32c_table_big[5][(crcw >> 40) & 0xff] ^ crc32c_table_big[6][(crcw >> 48) & 0xff] ^ crc32c_table_big[7][(crcw >> 56)];
			next += 8;
			len -= 8;
		}
		while (len >= 8);
